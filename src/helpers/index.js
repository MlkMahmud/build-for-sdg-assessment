...
export const estimateDollarsInFlight = ({ data, impact, severeImpact } = {}) => {
  ...
  const days = convertDurationToDays(data);
  const { avgDailyIncomeInUSD: avgDailyIncome } = data.region;

  impact[key] = impact.infectionsByRequestedTime * 0.65 * avgDailyIncome * days;
  severeImpact[key] = severeImpact.infectionsByRequestedTime * 0.65 * avgDailyIncome * days;
